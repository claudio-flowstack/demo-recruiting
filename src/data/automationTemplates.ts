...
/** Compact grid (108px row spacing) — used for templates with many horizontal phases */
const p2 = (col: number, row: number) => ({ x: 40 + col * 340, y: 58 + row * 108 });
...
export const WORKFLOW_TEMPLATES: AutomationSystem[] = [
  ...
  // ─── Recruiting Fulfillment (Compact) ───────────────────────────────────────
  {
    id: 'tpl-recruiting-compact',
    name: 'Recruiting Fulfillment (Compact)',
    description: 'Kompakter Recruiting-Workflow: Vom Kickoff über Onboarding und Knowledge Processing bis zur Asset-Produktion und Handover — ohne technische Hilfs-Nodes.',
    category: 'Recruiting',
    icon: 'users',
    status: 'draft',
    webhookUrl: '',
    nodes: [
      { id: 'rk1',  label: 'Formular eingereicht',   description: 'Neuer Recruiting-Kunde',              icon: 'logo-typeform',       type: 'trigger', ...p2(0, 1) },
      { id: 'rk2',  label: 'Slack: Neuer Kunde',     description: 'Team über neuen Kunden informieren',   icon: 'logo-slack',          type: 'output',  ...p2(1, 0) },
      { id: 'rk3',  label: 'Ordner & Templates',     description: 'Kundenordner und Vorlagen anlegen',    icon: 'logo-google-drive',   type: 'output',  ...p2(1, 1) },
      { id: 'rk4',  label: 'Onboarding-Termin',      description: 'Erstgespräch mit Kunden buchen',       icon: 'logo-calendly',       type: 'output',  ...p2(1, 2) },
      { id: 'rk5',  label: 'Gmail Bestätigung',      description: 'Terminbestätigung an Kunden senden',   icon: 'logo-gmail',          type: 'output',  ...p2(2, 0) },
      { id: 'rk6',  label: 'Onboarding Call',        description: 'Erstgespräch mit dem Kunden',          icon: 'video',               type: 'process', ...p2(2, 1) },
      { id: 'rk7',  label: 'KI-Transkription',       description: 'Gespräch transkribieren und analysieren', icon: 'logo-openai',      type: 'ai',      ...p2(3, 1) },
      { id: 'rk8',  label: 'Daten auslesen',         description: 'Key-Infos aus Transkript extrahieren', icon: 'scan',                type: 'ai',      ...p2(4, 1) },
      { id: 'rk9',  label: 'Employer Profile',       description: 'Arbeitgeber-Steckbrief erstellen',     icon: 'logo-google-docs',    type: 'output',  ...p2(5, 0) },
      { id: 'rk10', label: 'Stellenprofil & Personas', description: 'Rollen- und Zielgruppen-Profile',    icon: 'logo-google-docs',    type: 'output',  ...p2(5, 1) },
      { id: 'rk11', label: 'Ansprache & Compliance', description: 'Angle Matrix und Compliance-Regeln',   icon: 'logo-google-docs',    type: 'output',  ...p2(5, 2) },
      { id: 'rk12', label: 'Freigabe: Fakten',       description: 'Manuelle Faktenprüfung',               icon: 'shield-check',        type: 'process', ...p2(6, 1) },
      { id: 'rk13', label: 'Text generieren (LP)',   description: 'Landing-Page-Texte mit KI erstellen',  icon: 'logo-openai',         type: 'ai',      ...p2(7, 0) },
      { id: 'rk14', label: 'Landing Page',           description: 'Entwurf als Google Doc',               icon: 'logo-google-docs',    type: 'output',  ...p2(8, 0) },
      { id: 'rk15', label: 'Text generieren (Ads)',  description: 'Ad Copy pro Angle generieren',         icon: 'logo-openai',         type: 'ai',      ...p2(7, 1) },
      { id: 'rk16', label: 'Ad Copy Pack',           description: 'Alle Ad Copies gesammelt',             icon: 'logo-google-docs',    type: 'output',  ...p2(8, 1) },
      { id: 'rk17', label: 'Text generieren (CP)',   description: 'Creative Prompts generieren',          icon: 'logo-openai',         type: 'ai',      ...p2(7, 2) },
      { id: 'rk18', label: 'Creative Prompts',       description: 'Prompt-Pack als Google Doc',           icon: 'logo-google-docs',    type: 'output',  ...p2(8, 2) },
      { id: 'rk19', label: 'Freigabe: Copy',         description: 'Manuelle Copy-Prüfung',                icon: 'shield-check',        type: 'process', ...p2(9, 1) },
      { id: 'rk20', label: 'PDF Summary',            description: 'Alles als PDF zusammenfassen',         icon: 'file-type-2',         type: 'output',  ...p2(10, 1) },
      { id: 'rk21', label: 'Gmail an Client',        description: 'Übergabe-Mail an Kunden senden',      icon: 'logo-gmail',          type: 'output',  ...p2(11, 0) },
      { id: 'rk22', label: 'Drive speichern',        description: 'PDF im Kundenordner ablegen',          icon: 'logo-google-drive',   type: 'output',  ...p2(11, 1) },
      { id: 'rk23', label: 'Slack: Handover done',   description: 'Team über Übergabe informieren',       icon: 'logo-slack',          type: 'output',  ...p2(11, 2) },
      { id: 'rk24', label: 'Zeitplan (wöchentl.)',   description: 'Wiederkehrender Wochenrhythmus',       icon: 'calendar',            type: 'trigger', ...p2(12, 1) },
      { id: 'rk25', label: 'KPI Update',             description: 'Performance-Daten aktualisieren',      icon: 'logo-google-sheets',  type: 'process', ...p2(13, 0) },
      { id: 'rk26', label: 'Slack Weekly Report',    description: 'Wöchentlichen Report posten',          icon: 'logo-slack',          type: 'output',  ...p2(13, 1) },
    ],
    connections: [
      { from: 'rk1', to: 'rk2' }, { from: 'rk1', to: 'rk3' }, { from: 'rk1', to: 'rk4' },
      { from: 'rk4', to: 'rk5' }, { from: 'rk4', to: 'rk6' },
      { from: 'rk6', to: 'rk7' }, { from: 'rk7', to: 'rk8' },
      { from: 'rk8', to: 'rk9' }, { from: 'rk8', to: 'rk10' }, { from: 'rk8', to: 'rk11' },
      { from: 'rk9', to: 'rk12' }, { from: 'rk10', to: 'rk12' }, { from: 'rk11', to: 'rk12' },
      { from: 'rk12', to: 'rk13' }, { from: 'rk12', to: 'rk15' }, { from: 'rk12', to: 'rk17' },
      { from: 'rk13', to: 'rk14' }, { from: 'rk15', to: 'rk16' }, { from: 'rk17', to: 'rk18' },
      { from: 'rk14', to: 'rk19' }, { from: 'rk16', to: 'rk19' }, { from: 'rk18', to: 'rk19' },
      { from: 'rk19', to: 'rk20' },
      { from: 'rk20', to: 'rk21' }, { from: 'rk20', to: 'rk22' }, { from: 'rk20', to: 'rk23' },
      { from: 'rk24', to: 'rk25' }, { from: 'rk25', to: 'rk26' },
    ],
    groups: [
      { id: 'grk1', label: 'Phase 1 – Kickoff',              x: 15,   y: 18,  width: 620,  height: 364, color: 'blue' },
      { id: 'grk2', label: 'Phase 2 – Onboarding',            x: 695,  y: 18,  width: 620,  height: 256, color: 'blue' },
      { id: 'grk3', label: 'Phase 3 – Knowledge',             x: 1375, y: 18,  width: 960,  height: 364, color: 'purple' },
      { id: 'grk4', label: 'Phase 4 – Production',            x: 2395, y: 18,  width: 960,  height: 364, color: 'purple' },
      { id: 'grk5', label: 'Phase 5 – Handover',              x: 3415, y: 18,  width: 620,  height: 364, color: 'green' },
      { id: 'grk6', label: 'Phase 6 – Tracking',              x: 4095, y: 18,  width: 620,  height: 256, color: 'gray' },
    ],
    outputs: [],
    executionCount: 0,
  },
  // ─── Content Factory ───────────────────────────────────────────────────────
  {
    ...
    webhookUrl: '',
    nodes: [
      { id: 'cf1',  label: 'Formular eingereicht',   description: 'Neues Content-Thema eingegangen',       icon: 'logo-typeform',       type: 'trigger', ...p2(0, 2) },
      { id: 'cf2',  label: 'Duplikat-Check',         description: 'Doppelte Themen verhindern',             icon: 'copy-check',          type: 'process', ...p2(1, 2) },
      { id: 'cf3',  label: 'Slack: Neues Thema',     description: 'Team über neues Thema informieren',      icon: 'logo-slack',          type: 'output',  ...p2(1, 1) },
      { id: 'cf4',  label: 'KI-Analyse',             description: 'Quellen recherchieren, Kernthesen ableiten', icon: 'logo-openai',     type: 'ai',      ...p2(2, 2) },
      { id: 'cf5',  label: 'Research Brief',          description: 'Recherche-Ergebnis als Google Doc',     icon: 'logo-google-docs',    type: 'output',  ...p2(3, 1) },
      { id: 'cf6',  label: 'Aufteilen (4 Plattf.)',  description: '4 parallele Content-Linien starten',    icon: 'split',               type: 'process', ...p2(4, 2) },
      { id: 'cf7',  label: 'Text generieren (YT)',   description: 'YouTube Script: Hook → Struktur → CTA', icon: 'logo-openai',         type: 'ai',      ...p2(5, 0) },
      { id: 'cf8',  label: 'YouTube Script',          description: 'Script-Draft als Google Doc',           icon: 'logo-google-docs',    type: 'output',  ...p2(6, 0) },
      { id: 'cf9',  label: 'Text generieren (IG)',   description: 'Instagram Caption + Hook + Hashtags',   icon: 'logo-openai',         type: 'ai',      ...p2(5, 1) },
      { id: 'cf10', label: 'Instagram Draft',         description: 'Instagram-Entwurf als Google Doc',      icon: 'logo-google-docs',    type: 'output',  ...p2(6, 1) },
      { id: 'cf11', label: 'Text generieren (LI)',   description: 'LinkedIn Post: klar, B2B-fokussiert',   icon: 'logo-openai',         type: 'ai',      ...p2(5, 3) },
      { id: 'cf12', label: 'LinkedIn Draft',           description: 'LinkedIn-Entwurf als Google Doc',      icon: 'logo-google-docs',    type: 'output',  ...p2(6, 3) },
      { id: 'cf13', label: 'Text generieren (FB)',   description: 'Facebook Post: kurz + CTA',             icon: 'logo-openai',         type: 'ai',      ...p2(5, 4) },
      { id: 'cf14', label: 'Facebook Draft',           description: 'Facebook-Entwurf als Google Doc',      icon: 'logo-google-docs',    type: 'output',  ...p2(6, 4) },
      { id: 'cf15', label: 'Zusammenführen',          description: 'Alle 4 Plattform-Drafts fertig',       icon: 'git-merge',           type: 'process', ...p2(7, 2) },
      { id: 'cf16', label: 'Content Pack (Docs)',    description: 'Master-Dokument mit allen Drafts',      icon: 'logo-google-docs',    type: 'output',  ...p2(8, 1) },
      { id: 'cf17', label: 'Content Kalender',       description: 'Posting-Plan in Google Sheets',         icon: 'logo-google-sheets',  type: 'output',  ...p2(8, 3) },
      { id: 'cf18', label: 'Freigabe: Content',      description: 'Manuelle Prüfung vor Veröffentlichung', icon: 'shield-check',        type: 'process', ...p2(9, 2) },
      { id: 'cf19', label: 'Slack: Content fertig',  description: 'Team informieren: Content Pack ready',  icon: 'logo-slack',          type: 'output',  ...p2(10, 2) },
      { id: 'cf20', label: 'Timer',                  description: 'Nächster Posting-Slot abwarten',        icon: 'timer',               type: 'process', ...p2(11, 1) },
      { id: 'cf21', label: 'Benachrichtigung',       description: 'Post freigeben / planen',               icon: 'bell',                type: 'output',  ...p2(11, 2) },
      { id: 'cf22', label: 'Fehlerbehandlung',       description: 'Fehler im Workflow abfangen',           icon: 'shield-alert',        type: 'process', ...p2(0, 5) },
      { id: 'cf23', label: 'Slack: Error',            description: 'Content Factory Error melden',          icon: 'logo-slack',          type: 'output',  ...p2(1, 5) },
    ],
    connections: [
    ...
    groups: [
      { id: 'gcf1', label: 'Phase 1 – Input',            x: 15,   y: 126, width: 620,  height: 256, color: 'blue' },
      { id: 'gcf2', label: 'Phase 2 – Recherche',         x: 695,  y: 126, width: 620,  height: 256, color: 'blue' },
      { id: 'gcf3', label: 'Phase 3 – Produktion',        x: 1375, y: 18,  width: 1300, height: 580, color: 'purple' },
      { id: 'gcf4', label: 'Phase 4 – Packaging',         x: 2735, y: 126, width: 960,  height: 364, color: 'green' },
      { id: 'gcf5', label: 'Phase 5 – Scheduling',        x: 3755, y: 126, width: 280,  height: 256, color: 'gray' },
      { id: 'gcf6', label: 'Fehlerbehandlung',            x: 15,   y: 558, width: 620,  height: 148, color: 'orange' },
    ],
    outputs: [],
    executionCount: 0,
  },
  // ─── Buchhaltung & Rechnungen ──────────────────────────────────────────────
  {
    ...
    webhookUrl: '',
    nodes: [
      { id: 'bk1',  label: 'E-Mail empfangen',      description: 'Eingehende Rechnung per E-Mail',       icon: 'logo-gmail',          type: 'trigger', ...p2(0, 1) },
      { id: 'bk2',  label: 'Daten auslesen',         description: 'Betrag, Datum, Absender extrahieren',  icon: 'scan',                type: 'ai',      ...p2(1, 1) },
      { id: 'bk3',  label: 'Drive Ablegen',           description: 'Rechnung im Ordner /Finance/ speichern', icon: 'logo-google-drive', type: 'output',  ...p2(2, 0) },
      { id: 'bk4',  label: 'Finance Ledger',          description: 'Neue Zeile im Tracking-Sheet anlegen', icon: 'logo-google-sheets',  type: 'process', ...p2(2, 1) },
      { id: 'bk5',  label: 'Slack: Neue Rechnung',   description: 'Team über neue Eingangsrechnung informieren', icon: 'logo-slack',    type: 'output',  ...p2(2, 2) },
      { id: 'bk6',  label: 'Formular eingereicht',   description: 'Neue Rechnung / Angebot anlegen',      icon: 'logo-typeform',       type: 'trigger', ...p2(3, 1) },
      { id: 'bk7',  label: 'Docs Rechnung',           description: 'Rechnungs-Dokument aus Template',     icon: 'logo-google-docs',    type: 'output',  ...p2(4, 0) },
      { id: 'bk8',  label: 'PDF erstellen',           description: 'Rechnung als PDF generieren',          icon: 'file-type-2',         type: 'output',  ...p2(4, 1) },
      { id: 'bk9',  label: 'Gmail an Kunden',         description: 'PDF-Rechnung an Kunden senden',        icon: 'logo-gmail',          type: 'output',  ...p2(5, 0) },
      { id: 'bk10', label: 'Drive Outgoing',          description: 'PDF im Ordner /Outgoing/ speichern',   icon: 'logo-google-drive',   type: 'output',  ...p2(5, 1) },
      { id: 'bk11', label: 'Zeitplan (täglich)',     description: 'Tägliche Prüfung offener Rechnungen',  icon: 'calendar',            type: 'trigger', ...p2(6, 1) },
      { id: 'bk12', label: 'Sheets Lookup',           description: 'Offene Posten nach Fälligkeit suchen', icon: 'file-search',         type: 'process', ...p2(7, 1) },
      { id: 'bk13', label: 'Überfällig?',            description: 'Prüfen ob Rechnung überfällig ist',    icon: 'git-branch',          type: 'process', ...p2(8, 1) },
      { id: 'bk14', label: 'Gmail Erinnerung',       description: 'Zahlungserinnerung an Kunden senden',  icon: 'logo-gmail',          type: 'output',  ...p2(9, 0) },
      { id: 'bk15', label: 'Slack Hinweis',           description: 'Team über überfällige Rechnung informieren', icon: 'logo-slack',    type: 'output',  ...p2(9, 1) },
      { id: 'bk16', label: 'Sheets Status',           description: 'Status auf "Erinnert" setzen',         icon: 'logo-google-sheets',  type: 'process', ...p2(9, 2) },
      { id: 'bk17', label: 'Zeitplan (monatlich)',   description: 'Monatlicher Finance-Report',            icon: 'calendar',            type: 'trigger', ...p2(10, 1) },
      { id: 'bk18', label: 'KI-Analyse',             description: 'Ledger zusammenfassen: Summen & Trends', icon: 'logo-openai',        type: 'ai',      ...p2(11, 1) },
      { id: 'bk19', label: 'PDF Monatsreport',       description: 'Report als PDF generieren',             icon: 'file-type-2',         type: 'output',  ...p2(11, 0) },
      { id: 'bk20', label: 'Gmail intern',            description: 'Report per E-Mail intern versenden',   icon: 'logo-gmail',          type: 'output',  ...p2(12, 0) },
      { id: 'bk21', label: 'Slack: Report fertig',   description: 'Monatsreport im Slack posten',         icon: 'logo-slack',          type: 'output',  ...p2(12, 1) },
      { id: 'bk22', label: 'Fehlerbehandlung',       description: 'Fehler im Workflow abfangen',           icon: 'shield-alert',        type: 'process', ...p2(6, 4) },
      { id: 'bk23', label: 'Slack: Error',            description: 'Finance Workflow Error melden',         icon: 'logo-slack',          type: 'output',  ...p2(7, 4) },
    ],
    connections: [
    ...
    groups: [
      { id: 'gbk1', label: 'Phase 1 – Eingangsrechnungen',  x: 15,   y: 18,  width: 960,  height: 364, color: 'blue' },
      { id: 'gbk2', label: 'Phase 2 – Ausgangsrechnungen',   x: 1035, y: 18,  width: 960,  height: 256, color: 'blue' },
      { id: 'gbk3', label: 'Phase 3 – Tracking & Mahnwesen', x: 2055, y: 18,  width: 1300, height: 364, color: 'orange' },
      { id: 'gbk4', label: 'Phase 4 – Monatsabschluss',       x: 3415, y: 18,  width: 960,  height: 256, color: 'green' },
      { id: 'gbk5', label: 'Fehlerbehandlung',                x: 2055, y: 450, width: 620,  height: 148, color: 'orange' },
    ],
    outputs: [],
    executionCount: 0,
  },
  // ─── Coach Inbox Copilot ───────────────────────────────────────────────────
  {
    ...
    webhookUrl: '',
    nodes: [
      { id: 'ci1',  label: 'E-Mail empfangen',      description: 'Neue Anfrage im Postfach',             icon: 'logo-gmail',          type: 'trigger', ...p2(0, 1) },
      { id: 'ci2',  label: 'Automatisch sortieren',  description: 'Kategorie: FAQ / Buchung / Kundenfrage', icon: 'brain',             type: 'ai',      ...p2(1, 0) },
      { id: 'ci3',  label: 'Inbox Tracker',           description: 'Anfrage im Tracking-Sheet erfassen',   icon: 'logo-google-sheets',  type: 'process', ...p2(1, 1) },
      { id: 'ci4',  label: 'Slack: Neue Anfrage',    description: 'Team über Eingang informieren',         icon: 'logo-slack',          type: 'output',  ...p2(1, 2) },
      { id: 'ci5',  label: 'FAQ?',                   description: 'Ist es eine Standardfrage?',            icon: 'git-branch',          type: 'process', ...p2(2, 1) },
      { id: 'ci6',  label: 'Text generieren',         description: 'KI-Antwortentwurf für FAQ erstellen',  icon: 'logo-openai',         type: 'ai',      ...p2(3, 0) },
      { id: 'ci7',  label: 'Sheets: Needs Human',    description: 'Als "Mensch nötig" markieren',          icon: 'logo-google-sheets',  type: 'process', ...p2(3, 2) },
      { id: 'ci8',  label: 'Text-Template',           description: 'Ton, Signatur und CTA anwenden',       icon: 'type',                type: 'process', ...p2(4, 0) },
      { id: 'ci9',  label: 'Freigabe: Coach',        description: 'Coach prüft und bestätigt Antwort',    icon: 'shield-check',        type: 'process', ...p2(5, 1) },
      { id: 'ci10', label: 'Gmail senden',            description: 'Freigegebene Antwort versenden',       icon: 'logo-gmail',          type: 'output',  ...p2(6, 0) },
      { id: 'ci11', label: 'Sheets: Status',          description: 'Tracking-Status aktualisieren',        icon: 'logo-google-sheets',  type: 'process', ...p2(6, 1) },
      { id: 'ci12', label: 'Zeitplan (täglich)',     description: 'Tägliche Prüfung unbeantworteter Anfragen', icon: 'calendar',        type: 'trigger', ...p2(7, 1) },
      { id: 'ci13', label: 'Sheets Lookup',           description: 'Offene Anfragen > 24h suchen',         icon: 'file-search',         type: 'process', ...p2(8, 1) },
      { id: 'ci14', label: 'Slack Reminder',          description: 'Erinnerung: Antwort fällig',           icon: 'logo-slack',          type: 'output',  ...p2(9, 0) },
      { id: 'ci15', label: 'Benachrichtigung',       description: 'Push-Notification an Coach',            icon: 'bell',                type: 'output',  ...p2(9, 1) },
      { id: 'ci16', label: 'Fehlerbehandlung',       description: 'Fehler im Workflow abfangen',           icon: 'shield-alert',        type: 'process', ...p2(7, 3) },
      { id: 'ci17', label: 'Slack: Error',            description: 'Inbox Copilot Error melden',           icon: 'logo-slack',          type: 'output',  ...p2(8, 3) },
    ],
    connections: [
    ...
    groups: [
      { id: 'gci1', label: 'Phase 1 – Erfassung',          x: 15,   y: 18,  width: 620,  height: 364, color: 'blue' },
      { id: 'gci2', label: 'Phase 2 – Klassifizierung',     x: 695,  y: 18,  width: 620,  height: 364, color: 'purple' },
      { id: 'gci3', label: 'Phase 3 – Antwort & Freigabe',  x: 1375, y: 18,  width: 960,  height: 256, color: 'green' },
      { id: 'gci4', label: 'Phase 4 – Reminder',             x: 2395, y: 18,  width: 960,  height: 256, color: 'orange' },
      { id: 'gci5', label: 'Fehlerbehandlung',               x: 2395, y: 342, width: 620,  height: 148, color: 'orange' },
    ],
    outputs: [],
    executionCount: 0,
  },
  // ─── LinkedIn Outreach Assistant ───────────────────────────────────────────
  {
    ...
    webhookUrl: '',
    nodes: [
      { id: 'lo1',  label: 'Tabelle geändert',      description: 'Neuer Lead in Leads-Sheet',            icon: 'logo-google-sheets',  type: 'trigger', ...p2(0, 1) },
      { id: 'lo2',  label: 'Duplikat-Check',         description: 'Doppelte Leads verhindern',             icon: 'copy-check',          type: 'process', ...p2(1, 1) },
      { id: 'lo3',  label: 'Slack: Neuer Lead',      description: 'Team über neuen Lead informieren',      icon: 'logo-slack',          type: 'output',  ...p2(1, 0) },
      { id: 'lo4',  label: 'KI-Analyse',             description: '3 personalisierte Ansprache-Points ableiten', icon: 'logo-openai',   type: 'ai',      ...p2(2, 1) },
      { id: 'lo5',  label: 'Lead Brief (Docs)',      description: 'Personalisierungs-Brief als Google Doc', icon: 'logo-google-docs',   type: 'output',  ...p2(3, 1) },
      { id: 'lo6',  label: 'Text generieren',         description: 'Outreach-Message mit KI erstellen',    icon: 'logo-openai',         type: 'ai',      ...p2(4, 1) },
      { id: 'lo7',  label: 'Freigabe',               description: 'Message vor Versand prüfen',            icon: 'shield-check',        type: 'process', ...p2(5, 1) },
      { id: 'lo8',  label: 'Slack: Copy ready',      description: 'Message freigegeben und bereit',         icon: 'logo-slack',          type: 'output',  ...p2(6, 0) },
      { id: 'lo9',  label: 'Sheets Status',           description: 'Lead-Status auf "Approved" setzen',    icon: 'logo-google-sheets',  type: 'process', ...p2(6, 1) },
      { id: 'lo10', label: 'Warten (2 Tage)',        description: 'Auf Antwort warten',                    icon: 'clock',               type: 'process', ...p2(7, 1) },
      { id: 'lo11', label: 'Antwort?',               description: 'Hat der Lead geantwortet?',              icon: 'git-branch',          type: 'process', ...p2(8, 1) },
      { id: 'lo12', label: 'Text generieren (FU)',   description: 'Follow-up-Message erstellen',            icon: 'logo-openai',         type: 'ai',      ...p2(9, 0) },
      { id: 'lo13', label: 'Sheets: Follow-up',      description: 'Status auf "Follow-up sent" setzen',    icon: 'logo-google-sheets',  type: 'process', ...p2(9, 2) },
      { id: 'lo14', label: 'Google Calendar',         description: 'Termin-Event erstellen',                icon: 'calendar',            type: 'output',  ...p2(10, 1) },
      { id: 'lo15', label: 'Gmail Bestätigung',      description: 'Terminbestätigung + Agenda senden',     icon: 'logo-gmail',          type: 'output',  ...p2(11, 0) },
      { id: 'lo16', label: 'Slack: Termin gesetzt',  description: 'Team über gebuchten Termin informieren', icon: 'logo-slack',          type: 'output',  ...p2(11, 1) },
      { id: 'lo17', label: 'Sheets Pipeline',         description: 'Pipeline-Status auf "Booked" setzen',  icon: 'logo-google-sheets',  type: 'process', ...p2(11, 2) },
      { id: 'lo18', label: 'Zeitplan (wöchentl.)',   description: 'Wöchentliches KPI-Update',              icon: 'calendar',            type: 'trigger', ...p2(12, 1) },
      { id: 'lo19', label: 'Slack KPI Summary',      description: 'Outreach-KPIs im Slack posten',         icon: 'logo-slack',          type: 'output',  ...p2(13, 1) },
      { id: 'lo20', label: 'Fehlerbehandlung',       description: 'Fehler im Workflow abfangen',           icon: 'shield-alert',        type: 'process', ...p2(7, 4) },
      { id: 'lo21', label: 'Slack: Error',            description: 'Outreach Assistant Error melden',       icon: 'logo-slack',          type: 'output',  ...p2(8, 4) },
    ],
    connections: [
    ...
    groups: [
      { id: 'glo1', label: 'Phase 1 – Lead Intake',         x: 15,   y: 18,  width: 620,  height: 256, color: 'blue' },
      { id: 'glo2', label: 'Phase 2 – Personalization',      x: 695,  y: 126, width: 620,  height: 148, color: 'blue' },
      { id: 'glo3', label: 'Phase 3 – Message & Freigabe',   x: 1375, y: 18,  width: 960,  height: 256, color: 'purple' },
      { id: 'glo4', label: 'Phase 4 – Follow-up',            x: 2395, y: 18,  width: 960,  height: 364, color: 'orange' },
      { id: 'glo5', label: 'Phase 5 – Termin',               x: 3415, y: 18,  width: 620,  height: 364, color: 'green' },
      { id: 'glo6', label: 'Phase 6 – KPIs',                 x: 4095, y: 126, width: 620,  height: 148, color: 'gray' },
      { id: 'glo7', label: 'Fehlerbehandlung',               x: 2395, y: 450, width: 620,  height: 148, color: 'orange' },
    ],
    outputs: [],
    executionCount: 0,
  },
...
export const TEMPLATE_META_EN: Record<string, { name: string; description: string }> = {
  ...
  'tpl-recruiting-compact': { name: 'Recruiting Fulfillment (Compact)', description: 'Compact recruiting workflow: from kickoff through onboarding and knowledge processing to asset production and handover — without technical utility nodes.' },
...
export const TEMPLATE_NODE_EN: Record<string, { label: string; description: string }> = {
  ...
  // ─── Recruiting Fulfillment (Compact) ───
  'rk1':  { label: 'Form Submitted',           description: 'New recruiting client' },
  'rk2':  { label: 'Slack: New Client',          description: 'Inform team about new client' },
  'rk3':  { label: 'Folders & Templates',        description: 'Create client folder and templates' },
  'rk4':  { label: 'Onboarding Appointment',     description: 'Book initial meeting with client' },
  'rk5':  { label: 'Gmail Confirmation',          description: 'Send appointment confirmation to client' },
  'rk6':  { label: 'Onboarding Call',             description: 'Initial meeting with the client' },
  'rk7':  { label: 'AI Transcription',            description: 'Transcribe and analyze conversation' },
  'rk8':  { label: 'Extract Data',               description: 'Extract key info from transcript' },
  'rk9':  { label: 'Employer Profile',            description: 'Create employer fact sheet' },
  'rk10': { label: 'Job Profile & Personas',      description: 'Role and target group profiles' },
  'rk11': { label: 'Approach & Compliance',       description: 'Angle matrix and compliance rules' },
  'rk12': { label: 'Approval: Facts',            description: 'Manual fact checking' },
  'rk13': { label: 'Generate Text (LP)',          description: 'Create landing page copy with AI' },
  'rk14': { label: 'Landing Page',               description: 'Draft as Google Doc' },
  'rk15': { label: 'Generate Text (Ads)',         description: 'Generate ad copy per angle' },
  'rk16': { label: 'Ad Copy Pack',               description: 'All ad copies collected' },
  'rk17': { label: 'Generate Text (CP)',          description: 'Generate creative prompts' },
  'rk18': { label: 'Creative Prompts',            description: 'Prompt pack as Google Doc' },
  'rk19': { label: 'Approval: Copy',             description: 'Manual copy review' },
  'rk20': { label: 'PDF Summary',                description: 'Compile everything as PDF' },
  'rk21': { label: 'Gmail to Client',             description: 'Send handover email to client' },
  'rk22': { label: 'Save to Drive',              description: 'Store PDF in client folder' },
  'rk23': { label: 'Slack: Handover Done',        description: 'Inform team about handover' },
  'rk24': { label: 'Schedule (Weekly)',           description: 'Recurring weekly schedule' },
  'rk25': { label: 'KPI Update',                 description: 'Update performance data' },
  'rk26': { label: 'Slack Weekly Report',         description: 'Post weekly report' },
...
export const TEMPLATE_GROUP_EN: Record<string, string> = {
  ...
  'grc7': 'Tracking',
  // Recruiting Compact
  'grk1': 'Phase 1 – Kickoff',          'grk2': 'Phase 2 – Onboarding',
  'grk3': 'Phase 3 – Knowledge',        'grk4': 'Phase 4 – Production',
  'grk5': 'Phase 5 – Handover',         'grk6': 'Phase 6 – Tracking',
  // Content Factory
